function [H_actual_tap, Initial_Phase_for_all_rays, XPR, Rx_patterns, Tx_patterns, rhat_rx, wl, a_MS, a_BS, Num_ray, nTap, rand_coup_AOD, rand_coup_AOA, rand_coup_ZOD, rand_coup_ZOA] =  CDL_generation(para, very_first, Initial_Phase_for_all_rays, XPR, Rx_patterns, Tx_patterns, rhat_rx, wl, a_MS, a_BS, Num_ray, nTap, rand_coup_AOD, rand_coup_AOA, rand_coup_ZOD, rand_coup_ZOA)
    ...
    if very_first == true  
        ...
        for ind_tap = 1: nTap
            ...
            ZODs(1+Num_ray*(ind_tap-1):Num_ray*ind_tap,:) = repelem(ZODs_temp(ind_tap),Num_ray).' + offset_ang(rand_coup_ZOD)*para.c_ZSD;
        ...
        % no need for common delay thx to intial random phase....
        switch TxArrayType
            case 'ULA'
        ...
        % we now have all a_i and tau_i 
        % we will generate L-tap channel by sampling with 1/W
        % TODO: what if L is not long enough????
        ...
        for ind_actual_tap = 1:L
            for ind_cluster = 1:nTap
                ...
                a_b_curr_ray = BB_equiv(a, para.fc, curr_cluster_delay); 
        ...
        pathPowers_dB = CDL(:,2);
        pathPowers = 10.^( pathPowers_dB/10);
        W = 1/para.T_actual_sampling; % check!!
        vbar = user_speed * sph_to_car(phi_v,theta_v);    % UT velocity vector. (eq. 7.5-25)
        ...
        for ind_tap = 1:nTap
            b = 1;
            for ind_ray = 1:Num_ray
                % reused generated initial phases
                ...
                if para.Tx_pol == 1 && para.Rx_pol == 1
                    ...
                    msg = 'Not implemented for scalar channel case';
                    error(msg)
                end
            end
            H(:, :, ind_tap) = sqrt(pathPowers(ind_tap)/Num_ray)*H(:, :, ind_tap);
    ...
    function [theta_LCS, phi_LCS] = GCS_to_LCS(alpha, beta, gamma, theta_GCS , phi_GCS) % GCS to LCS. 38.802, 7.1.
        % GCS: Global Coordinate System, LCS: Local Coordinate System
